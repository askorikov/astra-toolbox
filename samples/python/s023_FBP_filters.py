...
proj_geom = astra.create_proj_geom(
    'parallel', 1.0, N, np.linspace(0, np.pi, 180, False)
)
...
fullFilterSize = 2*N
kernel = np.append(
    np.linspace(0, 1, fullFilterSize//2, endpoint=False),
    np.linspace(1, 0, fullFilterSize//2, endpoint=False)
)
...
plt.imshow(rec_RL)
plt.figure()
plt.imshow(rec_filter)
plt.figure()
plt.imshow(rec_kernel)
plt.show()
